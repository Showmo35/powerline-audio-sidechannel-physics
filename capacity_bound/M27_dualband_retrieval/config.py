...
class PLFConfig:
    ...
    # ── LOW-band encoder (raw 32 kHz / 0-16 kHz → condition tokens at frame rate) ─
    enc_strides: tuple = (8, 5, 4, 2)     # prod = 320 = in_sr / fps
    enc_kernels: tuple = (33, 17, 9, 5)
    enc_channels: tuple = (128, 192, 256, 384)
    enc_tf_layers: int = 4

    # ── HIGH-band encoder (FULL 200 kHz raw → condition tokens) ────────────────
    # M27 dual-band: a second learned front-end reads the FULL-rate 200 kHz window
    # (all of 0-100 kHz — every mains-harmonic sideband M22 discarded), and its
    # tokens are FUSED with the low-band tokens.  prod(hi_strides)=2000=cap_sr/fps.
    dualband: bool = True
    hi_sr: int = 200_000                  # full capture rate fed to the high stream
    hi_strides: tuple = (10, 10, 5, 2, 2) # prod = 2000 = cap_sr / fps
    hi_kernels: tuple = (41, 21, 11, 5, 5)
    hi_channels: tuple = (64, 128, 192, 256, 384)
    hi_enc_tf_layers: int = 2
    ...
    @property
    def hi_len(self):
        return int(round(self.win_s * self.hi_sr))    # 4 s * 200 kHz = 800000 samples
