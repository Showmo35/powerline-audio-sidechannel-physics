...
def diff_sample(model, raw, steps):
    ...
    `raw` may be a (raw_lo, raw_hi) pair for the dual-band encoder."""
    ref = raw[0] if isinstance(raw, (tuple, list)) else raw
    B, device = ref.shape[0], ref.device
    c = model.encode(raw)
    x = torch.randn(B, CFG.n_mels, CFG.n_frames, device=device)
    dt = 1.0 / steps
...
def knn_eval(model, gallery_emb, gallery_y, test_loader, device, mean, std, use_amp):
    ...
    for b in test_loader:
        raw = (b['raw_lo'].to(device), b['raw_hi'].to(device))
...
def main():
    ...
    while step < args.steps:
        ...
        raw = (b['raw_lo'].to(device, non_blocking=True),
               b['raw_hi'].to(device, non_blocking=True))          # dual-band pair
