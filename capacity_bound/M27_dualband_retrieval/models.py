...
class PowerlineEncoder(nn.Module):
    """Raw powerline window (B, L) → condition tokens (B, T, d_model).
    kernels/strides/channels/tf_layers default to the LOW-band (cfg.enc_*) config;
    pass overrides to build the HIGH-band (full 200 kHz) stream."""
    def __init__(self, cfg, kernels=None, strides=None, channels=None, tf_layers=None):
        super().__init__()
        kernels = kernels or cfg.enc_kernels
        strides = strides or cfg.enc_strides
        channels = channels or cfg.enc_channels
        tf_layers = cfg.enc_tf_layers if tf_layers is None else tf_layers
        ...
        self.proj = nn.Conv1d(channels[-1], cfg.d_model, 1)
...
class PLF(nn.Module):
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        d = cfg.d_model
        self.encoder = PowerlineEncoder(cfg)                     # LOW band (0-16 kHz)
        self.dualband = getattr(cfg, 'dualband', False)
        if self.dualband:                                        # HIGH band (full 200 kHz)
            self.encoder_high = PowerlineEncoder(
                cfg, kernels=cfg.hi_kernels, strides=cfg.hi_strides,
                channels=cfg.hi_channels, tf_layers=cfg.hi_enc_tf_layers)
            self.fuse = nn.Linear(2 * d, d)                      # fuse low+high tokens
    ...
    def encode(self, raw):
        """raw: (B, in_len) for single-band, or (raw_lo, raw_hi) for M27 dual-band.
        Dual-band fuses the LOW (0-16 kHz) and HIGH (full 200 kHz) token streams."""
        if self.dualband:
            raw_lo, raw_hi = raw
            c_lo = self.encoder(raw_lo)              # (B, T, d)
            c_hi = self.encoder_high(raw_hi)         # (B, T, d)
            return self.fuse_norm(self.fuse(torch.cat([c_lo, c_hi], dim=-1)))
    ...
    def sample(self, raw, steps=None, cfg_scale=None):
        ...
        ref = raw[0] if self.dualband else raw
        B, dev = ref.shape[0], ref.device
        c = self.encode(raw)
        c_null = self.null_cond[None, None, :].expand_as(c)
        x = torch.randn(B, cfg.n_mels, cfg.n_frames, device=dev)
        dt = 1.0 / steps
