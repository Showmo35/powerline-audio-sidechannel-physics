...
class WordSet(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        # M27 DUAL-BAND: one disk read → full 200 kHz (high stream) + decimated
        # 0-16 kHz (low stream); each RMS-normalized and padded to its length.
        full, low = io.read_bin_window_dualband(CFG.bin_path(ch), (s - ctx) + lag,
                                                CFG.win_s, CFG.cap_sr, CFG.in_sr)
        low = np.asarray(low, np.float32)
        low = low / (np.sqrt(np.mean(low ** 2)) + 1e-8)
        x_lo = np.zeros(CFG.in_len, np.float32)
        x_lo[:min(len(low), CFG.in_len)] = low[:CFG.in_len]

        full = np.asarray(full, np.float32)
        full = full / (np.sqrt(np.mean(full ** 2)) + 1e-8)
        x_hi = np.zeros(CFG.hi_len, np.float32)
        x_hi[:min(len(full), CFG.hi_len)] = full[:CFG.hi_len]
        ...
        return {'raw_lo': torch.from_numpy(x_lo), 'raw_hi': torch.from_numpy(x_hi),
                'mel_word': mw, 'mel_full': mf, 'frs': frs, 'label': y}


def collate(b):
    return {
        'raw_lo':   torch.stack([x['raw_lo'] for x in b]),      # (B, in_len)   0-16 kHz
        'raw_hi':   torch.stack([x['raw_hi'] for x in b]),      # (B, hi_len)   full 200 kHz
