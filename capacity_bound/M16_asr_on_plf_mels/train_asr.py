#!/usr/bin/env python3
"""
train_asr.py — train the char-CTC ASR on generated ('gen') or real ('real') mels.

Same manifest, same utterance filter, same chunk split for both — the ONLY
difference between the twin runs is the mel source, so their held-out WER gap
measures exactly how much lexical content the PLF-generated mels carry.

Eval each epoch = greedy-decode WER + CER on the held-out chunks, with a few
sample (ref, hyp) pairs printed.  best.pt is selected by test WER; last.pt
supports --resume.
"""
...
def spec_augment(mel, mel_lens, cfg=CFG):
    """In-place-ish SpecAugment on a padded batch of standardised mels."""
...
def main():
    ...
    ap.add_argument('--input', choices=('gen', 'real'), required=True)
    ...
    out_dir = args.out_dir or os.path.join(OUT_DIR, f'asr_{args.input}')
    ...
    tr_ds = D.MelText(tr_rows, args.input)
    te_ds = D.MelText(te_rows, args.input)
    print(f'[data] input={args.input}  train={len(tr_ds)}  test={len(te_ds)} utts')
    ...
    print(f'[model] ASR-CTC params={model.count_params():,}')
    ...
    step, start_ep, best_wer = 0, 0, float('inf')
    ...
    if args.resume and os.path.exists(last_path):
        ...
        step, start_ep, best_wer = ck['step'], ck['epoch'] + 1, ck['best_wer']
        log = ck['log']
        print(f'[resume] epoch={start_ep} step={step} best_wer={best_wer:.3f}')
    ...
    for ep in range(start_ep, args.epochs):
        ...
        state = {'model': model.state_dict(), 'opt': opt.state_dict(),
                 'step': step, 'epoch': ep, 'best_wer': min(best_wer, ev['wer']),
                 'mean': mean, 'std': std, 'log': log}
        torch.save(state, last_path)
        if ev['wer'] < best_wer:
            best_wer = ev['wer']
            torch.save(state, best_path)
            print(f'    ↑ new best WER={best_wer*100:.1f}% → saved')
        json.dump(log, open(os.path.join(out_dir, 'train_log.json'), 'w'), indent=1)

    print(f'[done] input={args.input}  best test WER={best_wer*100:.1f}%')
