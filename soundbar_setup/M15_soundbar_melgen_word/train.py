#!/usr/bin/env python3
"""
train.py — joint flow-matching + CTC word-loss trainer for PLF-W (M15).

  total = flow_matching + lambda_ctc · CTC(word)
Eval: samples mels (few-step ODE) → mel_r / env_r on held-out chunks, and
CTC-greedy-decodes the word head → WER vs the true LibriSpeech text.
"""
...
def evaluate(model, loader, device, mean, std, steps, cfg_scale, max_batches=3):
    ...
    for bi, b in enumerate(loader):
        if bi >= max_batches:
            break
        raw = b['raw'].to(device); tgt = b['mel'].to(device); vf = b['vframes']
    ...
    return {'mel_l1': L1 / nb, 'mel_r': R / nb, 'env_r': E / nb,
...
def main():
    ...
    tr_rows, te_rows = D.split_rows(rows, CFG)
    tr_ds = D.PLFWUtterances(tr_rows, CFG); te_ds = D.PLFWUtterances(te_rows, CFG)
    print(f'[data] train utts={len(tr_ds)}  test utts={len(te_ds)}  '
          f'a_len={CFG.a_len} ({CFG.cap_sr}Hz full band)  mel=[{CFG.n_mels},{CFG.n_frames}]')
    print('[stats] estimating mel mean/std …')
    mean, std = D.estimate_mel_stats(tr_ds, n=384)
    ...
    print(f'[model] PLF-W params={model.count_params():,}  lambda_ctc={args.lambda_ctc}')
    ...
    def save(path, ep, extra=None):
        ...
        d = {'model': model.state_dict(), 'ema': ema.shadow.state_dict(), 'opt': opt.state_dict(),
             ...
             'mel_mean': mean, 'mel_std': std, 'step': step, 'epoch': ep, 'best': best}
    ...
    for ep in range(start_ep, args.epochs):
        ...
        for b in tr_dl:
            ...
            mel = (b['mel'].to(device, non_blocking=True) - mean) / std
            vf = b['vframes'].to(device); lab = b['labels'].to(device); ll = b['label_lengths'].to(device)
            with torch.autocast('cuda', dtype=torch.float16, enabled=(device == 'cuda')):
                flow, ctc = model.losses(raw, mel, vf, lab, ll)
            ...
            if step % args.eval_every == 0 or step == total_steps:
                ...
                print(f'[eval] step{step}  mel_r={m["mel_r"]:.3f} env_r={m["env_r"]:.3f} '
                      f'WER={m["wer"]*100:.1f}% (n={m["n_words"]})')
                for r, h in samples:
                    print(f'    REF: {r[:70]}\n    HYP: {h[:70]}')
                log['evals'].append({'step': step, **m})
                score = m['wer'] + m['mel_l1'] * 0.0  # select by WER (word-focused goal)
                if score < best:
                    best = score; save(os.path.join(args.out_dir, 'best.pt'), ep, {'metrics': m})
    ...
    print(f'\n══ M15 PLF-W ══  params={model.count_params():,}  best WER={best*100:.1f}%  '
