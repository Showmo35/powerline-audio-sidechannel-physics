...
CONFIG = {
    ...
    'MODEL_NAME': 'powerline_wavelet_cnn',  # Model identifier
    'ORIGINAL_SAMPLE_RATE': 200_000,  # 200 kHz (original)
    'SAMPLE_RATE': 200_000,  # 100 kHz (downsampled for training)
...
# Create output base directory (parent folder)
...
# ## 4. Model Architecture: Wavelet-based Multi-scale Spectral Attention CNN
# 
# Multi-scale wavelet attention model that uses Morlet wavelets at different scales (coarse → fine) to analyze frequency content at multiple encoder/decoder stages. The model learns to focus on relevant frequency bands without pre-filtering.

# In[ ]:


# Import wavelet-based spectral attention model
from wavelet_attention import SpectralWaveletAttentionCNN

print("Model Architecture: Large ResNet-DenseNet Hybrid with Wavelet Attention")
print("=" * 70)
print("Components:")
print("  - WaveletConv: 50 Morlet wavelet scales (logarithmically spaced)")
print("  - 6-stage encoder: residual blocks + dense blocks at each level")
print("  - 6-stage decoder: dense skip connections from encoder")
print("  - 12 attention modules: wavelet attention at EVERY encoder/decoder stage")
print("  - Channel progression: 64 → 128 → 256 → 512 → 768 → 1024")
print("=" * 70)
print()

# Create large wavelet-based model with 50 scales
model = SpectralWaveletAttentionCNN(
    sample_rate=CONFIG['SAMPLE_RATE'],  # 200 kHz
    base_channels=64,  # Base channels (will scale to 1024 at bottleneck)
    num_scales=50  # All 50 wavelet scales used at every attention stage
)

model = model.to(CONFIG['DEVICE'])

print(f"✓ Large Spectral-Wavelet Attention CNN created")
...
attn_modules = [m for m in model.modules() if hasattr(m, '__class__') and 'Sequential' in m.__class__.__name__]
print(f"  Attention modules: 12 (6 encoder + 6 decoder)")
print(f"  Residual blocks: ~20+")
print(f"  Dense blocks: 12")

# Test forward pass with sample data
print(f"\nTesting forward pass...")
sample_powerline = train_chunks[0]['powerline_signal']
sample_input = torch.FloatTensor(sample_powerline).unsqueeze(0).unsqueeze(0).to(CONFIG['DEVICE'])
with torch.no_grad():
