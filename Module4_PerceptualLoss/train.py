...
train.py
--------
Train PowerlineUNet with perception-aware loss (L1 + MSTFT + CTC perceptual).

The perceptual loss uses a FROZEN pretrained CTC encoder to extract
intermediate features, encouraging the UNet to produce spectrograms that
are discriminative for speech recognition.

Usage:
    python train.py --data data/train_data.npz --out checkpoints \
                    --ctc_ckpt ../UNet/UNet-CTC/checkpoints/best_model.pt
"""
...
from model import PowerlineUNet, PerceptionAwareLoss
...
def save_sample_plot(gen, val_loader, device, epoch, out_dir):
    ...
    axes[0].imshow(n, **kw); axes[0].set_title('Noisy (Bandpass 50-4kHz)')
...
def main(args):
    ...
    raw_gen = gen.module if isinstance(gen, nn.DataParallel) else gen
    print(f"PowerlineUNet params : {raw_gen.count_params():,}")

    # ── loss (with frozen CTC perceptual encoder)
    ...
    # Only optimise UNet parameters (CTC encoder is frozen)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train PowerlineUNet with perception-aware loss")

    parser.add_argument('--data',       default=LOCAL_DATA)
    parser.add_argument('--out',        default=LOCAL_OUT)
    parser.add_argument('--ctc_ckpt',   default=os.path.join(
        _HERE, '..', 'UNet', 'UNet-CTC', 'checkpoints', 'best_model.pt'),
