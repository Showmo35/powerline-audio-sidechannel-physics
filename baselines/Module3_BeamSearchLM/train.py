...
Dual checkpointing: best_loss_model.pt and best_cer_model.pt.
Full validation CER computed every epoch. Early stopping on CER.
...
def validate_full(model, loader, device):
    """Full validation: compute loss and CER over ALL val samples."""
    model.eval()
    ctc_loss_fn = nn.CTCLoss(blank=BLANK_IDX, zero_infinity=True)
    total_loss, n_batches = 0.0, 0
    all_preds, all_refs = [], []
    ...
    for mels, texts, text_lens in loader:
        ...
        # Decode ALL batches for full CER
...
def main(args):
    ...
    print(f"\nTraining for up to {args.epochs} epochs "
          f"(patience={args.patience} on CER)\n")

    for epoch in range(1, args.epochs + 1):
        t0 = time.time()
        train_loss = train_epoch(model, train_loader, optimizer, device)

        # Full validation CER every epoch
        ...
        if val_loss < best_val_loss:
            ...
            print(f"  ** New best LOSS (val_loss={val_loss:.4f}, CER={val_cer:.4f})")

        # Checkpoint: best CER
        cer_improved = val_cer < best_val_cer
        if cer_improved:
            ...
            print(f"  ** New best CER  (val_loss={val_loss:.4f}, CER={val_cer:.4f})")
        else:
            patience_counter += 1
        ...
        if patience_counter >= args.patience:
            print(f"\nEarly stopping at epoch {epoch} "
                  f"(patience={args.patience}, best CER={best_val_cer:.4f})")
            break
    ...
    print(f"  best_loss_model.pt  best_cer_model.pt")
