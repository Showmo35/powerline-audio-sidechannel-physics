#!/bin/bash
#SBATCH --job-name=Recon_Train_Infer
...
# ── Training ─────────────────────────────────────────────────────────────────
echo ""
echo ">>> TRAINING (L1 + MSTFT, temporal split, no Whisper)"
echo "----------------------------------------------------------------------"

python3 -u train.py \
    --data "$UNET_DIR/data/train_data.npz" \
    --out  "$UNET_DIR/checkpoints_recon" \
    --epochs 300 \
    --lr 3e-4 \
    --batch 32 \
    --base_ch 64 \
...
python3 -u inference.py \
    --mode val \
    --ckpt "$UNET_DIR/checkpoints_recon/best_model.pt" \
    --out  "$UNET_DIR/inference_output_recon" \
    --data "$UNET_DIR/data/train_data.npz" \
    --n_samples 10
