...
train.py
--------
Train the Soundbar UNet reconstruction model on sharded spectrogram data.
"""
...
from model import GeneratorLoss, PowerlineUNet
...
def train_one_epoch(gen, loader, optimizer, loss_fn, device, scaler=None):
    gen.train()
    totals = {"loss": 0.0, "l1": 0.0, "mstft": 0.0}
...
def save_sample_plot(gen, val_loader, device, epoch, out_dir):
    ...
    axes[0].imshow(n, **kw)
    axes[0].set_title("Noisy Soundbar")
    axes[1].imshow(c, **kw)
    axes[1].set_title("Clean Audio")
    axes[2].imshow(p, **kw)
    axes[2].set_title(f"Predicted epoch {epoch}")
    for ax in axes:
...
def main(args):
    ...
    history = {"loss": [], "val_loss": [], "l1": [], "val_l1": [], "mstft": [], "val_mstft": [], "lr": []}
    best_val = float("inf")
    patience_counter = 0
    ...
    print(f"Loss weights: L1={args.l1_w}  MSTFT={args.mstft_w}")
    print(f"Early stopping patience: {args.patience}\n")
    print(f"{'Ep':>4}  {'Loss':>8}  {'Val_L':>8}  {'L1':>7}  {'MSTFT':>7}  {'LR':>9}  {'Time':>6}")
    print("-" * 62)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            f"{epoch:4d}  {tr['loss']:8.5f}  {vl['loss']:8.5f}  {tr['l1']:7.4f}  {tr['mstft']:7.4f}  "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Soundbar UNet")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Disable shard-local batching for sharded datasets (kept for compatibility).",
    )
