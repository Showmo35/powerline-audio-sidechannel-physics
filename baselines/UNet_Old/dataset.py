...
class PowerlineDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            ...
            f_start = torch.randint(0, n_mels - 10, (1,)).item()
            f_width = torch.randint(1, 10, (1,)).item()
            noisy[0, f_start:f_start+f_width, :] = noisy.min()

            # Time masking: zero out up to 20 frames
            n_frames = noisy.shape[2]
            if n_frames > 20:
                t_start = torch.randint(0, n_frames - 20, (1,)).item()
                t_width = torch.randint(1, 20, (1,)).item()
