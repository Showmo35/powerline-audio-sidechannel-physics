...
sys.path.insert(0, '<REPO_ROOT>/M14_new_setup_soundbar_melgen')
cfg = importlib.import_module('config').CFG
import dataset as Dm
m14 = importlib.import_module('models').build(cfg).to(dev)
ck = torch.load('<REPO_ROOT>/M14_new_setup_soundbar_melgen/'
...
fig.tight_layout()
out = '<REPO_ROOT>/Capture_Analysis/hello_mels.png'
